---
title: "Project"
author: "Muhammad Oscar Fileas J"
date: "2024-12-02"
...
# UI
ui <- fluidPage(
    titlePanel("Aplikasi Rekomendasi Sewa Mobil"),
    sidebarLayout(
        sidebarPanel(
            fileInput("file", "Unggah Dataset (Excel)", accept = c(".xlsx")),
            textInput("keyword", "Masukkan Keyword (misal: tipe mobil, fitur, dll.)", ""),
            numericInput("budget", "Masukkan Harga Maksimal (IDR)", value = 500000, min = 0),
            actionButton("submit", "Cari"),
            br(),
            helpText("Unggah dataset Excel dengan kolom yang mencakup nama kendaraan, fitur, harga, dan lainnya.")
        ),
        mainPanel(
            tabsetPanel(
                tabPanel("Hasil Rekomendasi",
                         h4("Rekomendasi Mobil"),
                         DTOutput("recommend_table")),
                tabPanel("Data Mentah",
                         h4("Data Asli yang Diunggah"),
                         DTOutput("raw_data"))
            )
        )
    )
...
server <- function(input, output, session) {
    ...
    dataset <- reactive({
        req(input$file)  # Pastikan file sudah diunggah
        read_excel(input$file$datapath)
    })
    
    # Data preprocessing
    processed_data <- reactive({
        req(dataset())  # Pastikan dataset telah berhasil dibaca
        data <- dataset() %>%
        ...
        # Membersihkan data harga dan rating
        data$Harga <- as.numeric(gsub("IDR ", "", gsub("\\.", "", data$Harga)))
        data$Rating <- as.numeric(gsub(",", ".", gsub(".*\\s", "", data$Rating)))
        data$Review <- as.numeric(gsub("[^0-9]", "", data$Review))
        return(data)
    })
    
    # Filter berdasarkan keyword dan harga
    ...
    output$recommend_table <- renderDT({
        req(recommendation())  # Pastikan ada hasil rekomendasi
        datatable(recommendation())
    })
    
    # Output data mentah
    output$raw_data <- renderDT({
        req(processed_data())  # Pastikan data telah diproses
